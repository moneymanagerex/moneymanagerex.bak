...
void mmHomePagePanel::displayCheckingAccounts(mmHTMLBuilder& hb, double& tBalance, double& tIncome, double& tExpenses, wxDateTime& dtBegin, wxDateTime& dtEnd)
{
    ...
    for (size_t iAdx = 0; iAdx < core_->accountList_.accounts_.size(); ++iAdx)
    {
        ...
        if (pCA && pCA->status_== mmAccount::MMEX_Open)
        {
            ...
            double reconciledBal = pCA->initialBalance_ + core_->bTransactionList_.getReconciledBalance(pCA->accountID_, mmIniOptions::ignore_future_transactions_);
            ...
            if ( frame_->expandedBankAccounts() ) 
            {
                double income = 0.0;
                double expenses = 0.0;
                core_->bTransactionList_.getExpensesIncome(pCA->accountID_, expenses, income, false, dtBegin, dtEnd, mmIniOptions::ignore_future_transactions_);
...
void mmHomePagePanel::displayTermAccounts(mmHTMLBuilder& hb, double& tBalance, double& tIncome, double& tExpenses, wxDateTime& dtBegin, wxDateTime& dtEnd)
{
    ...
    for (size_t iAdx = 0; iAdx < core_->accountList_.accounts_.size(); ++iAdx)
    {
        ...
        if (pTA && pTA->status_== mmAccount::MMEX_Open)
        {
            ...
            double reconciledBal = pTA->initialBalance_ + core_->bTransactionList_.getReconciledBalance(pTA->accountID_, mmIniOptions::ignore_future_transactions_);
            double rate = pCurrencyPtr->baseConv_;
            ...
            if ( frame_->expandedTermAccounts() )
            {
                double income = 0;
                double expenses = 0;
                core_->bTransactionList_.getExpensesIncome(pTA->accountID_, expenses, income, false, dtBegin, dtEnd, mmIniOptions::ignore_future_transactions_);
