...
wxString mmCleanString( const wxString& orig )
{
    ...
    toReturn.Replace( wxT( "\"" ), wxT( "" ) );     // remove quotes from string
    return toReturn;
}
//----------------------------------------------------------------------------
...
int mmImportCSV( mmCoreDB* core )
{
    ...
    if ( scd.ShowModal() == wxID_OK ) {
        ...
        wxString fileName = wxFileSelector( _( "Choose CSV data file to import" ),
                                wxEmptyString, wxEmptyString, wxEmptyString, chooseExt, wxFD_FILE_MUST_EXIST );
        ...
        if ( !fileName.empty() ) {
            ...
            while ( !input.Eof() ) {
                ...
                if ( tkz.HasMoreTokens() )
                    dt = mmCleanString( tkz.GetNextToken() );
                else {
                ...
                if ( tkz.HasMoreTokens() )
                    ...
                    log << _( "Line : " ) << countNumTotal << _( " missing transaction type, skipping." ) << endl;
                    continue;
                }
                ...
                // Assuming that category is not a number. If it is, it could be part of the amount
                if ( categ.IsNumber() ){
                    log << _( "Line : " ) << countNumTotal << _( " check delimiters. Possible amount error, skipping." ) << endl;
                    continue;
                }
                ...
                if ( dt.Trim().IsEmpty() || payee.Trim().IsEmpty() || type.Trim().IsEmpty() || amount.Trim().IsEmpty() ) {
                    log << _( "Line : " ) << countNumTotal 
                        << _( "one of the following fields: date, payee, transaction type, amount strings is empty, skipping" )
                        << endl;
                    continue;
                }

                if ( type.Cmp( wxT( "Withdrawal" ) ) && type.Cmp( wxT( "Deposit" ) ) ) {
                    log << _( "Line : " ) << countNumTotal << _( " unrecognized transaction type, skipping." ) << endl;
                    continue;
                }
                ...
                if ( transfer == wxT( "T" ) ) {
                    ...
                    if ( core->accountList_.accountExists( payee ) ) {
                        ...
                        log << _( "Line : " ) << countNumTotal << _( " transfer transaction not imported." ) << endl;
                    }
                }
        ...
        if ( !fileName.IsEmpty() ) {
            ...
            while ( !input.Eof() ) {
                ...
                if ( dt.Trim().IsEmpty() || payee.Trim().IsEmpty() ||
                     ...
                     categ.Trim().IsEmpty() ) {
                    log << _( "Line : " ) << countNumTotal
                        << _( " One of the following fields: date, payee, transaction type, amount, category strings is empty, skipping" ) 
                        << endl;
                    continue;
                }
                ...
                if ( chr == NULL ) {
                    log << _( "Line : " ) << countNumTotal << _( " invalid date, skipping." ) << endl;
                    continue;
                }
