...
void mmGUIFrame::createBudgetingPage(int budgetYearID)
{
    ...
    else
    {
        ...
        budgetingPage_ = new mmBudgetingPanel(m_db.get(), m_core.get(), this, budgetYearID,
...
{
    const int currencyID = m_core->currencyList_.getBaseCurrencySettings();
    const wxString base_symbol = m_core->currencyList_.getCurrencySharedPtr(currencyID)->currencySymbol_;
    ...
    for (int idx = 0; idx < (int)m_core->currencyList_.currencies_.size(); idx++)
    {
        ...
        if (!currency_symbol.IsEmpty())
        {
            ...
            double old_rate = m_core->currencyList_.currencies_[idx]->baseConv_;
            ...
            msg << valueStr << wxT("\n");
            m_core->currencyList_.currencies_[idx]->baseConv_ = new_rate;
