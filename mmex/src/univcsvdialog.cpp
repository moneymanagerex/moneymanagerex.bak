...
BEGIN_EVENT_TABLE(mmUnivCSVDialog, wxDialog)
    ...
    EVT_CHOICE(ID_UNIVCSVDIALOG_DATE_FORMAT, mmUnivCSVDialog::OnDateFormatChanged)
END_EVENT_TABLE()
...
{
    wxSizerFlags flags;
    flags.Align(wxALIGN_LEFT|wxALIGN_CENTER_VERTICAL).Border(wxLEFT|wxTOP, 5);
   ...
   //Arranger Area
    ...
    //Move Up button
    wxButton* itemButton_MoveUp = new wxButton(itemPanel_Arranger, wxID_UP);
    itemBoxSizer_Arranger->Add(itemButton_MoveUp, flags);
    itemButton_MoveUp -> SetToolTip (_("Move Up"));

    //Move down button
    wxButton* itemButton_MoveDown = new wxButton(itemPanel_Arranger, wxID_DOWN);
    itemBoxSizer_Arranger->Add(itemButton_MoveDown, flags);
    itemButton_MoveDown -> SetToolTip (_("Move Down"));

    //Load Template button
    wxButton* itemButton_Load = new wxButton(itemPanel_Arranger, wxID_OPEN);
    ...
    if (this->is_importer_)
    {
        ...
        m_text_ctrl_ = new wxTextCtrl(itemPanel6, ID_FILE_NAME,
            wxEmptyString, wxDefaultPosition, wxSize(300, -1), wxTE_PROCESS_ENTER );
        itemBoxSizer7->Add(m_text_ctrl_, 1, wxALL|wxGROW, 5);
        m_text_ctrl_->Connect(ID_FILE_NAME, wxEVT_COMMAND_TEXT_UPDATED,
            wxCommandEventHandler(mmUnivCSVDialog::OnFileNameChanged), NULL, this);
        m_text_ctrl_->Connect(ID_FILE_NAME, wxEVT_COMMAND_TEXT_ENTER,
            wxCommandEventHandler(mmUnivCSVDialog::OnFileNameEntered), NULL, this);
    ...
    wxStaticText* itemStaticText6 = new wxStaticText(itemPanel7, wxID_ANY,
        _("Account  :"));
    itemBoxSizer8->Add(itemStaticText6, flags);
    ...
    wxStaticText* itemStaticText66 = new wxStaticText(itemPanel7, wxID_ANY, wxString() << ("    ") << _("Date Format") << (": "), wxDefaultPosition, wxDefaultSize, 0);
    ...
    m_radio_box_ = new wxRadioBox(this, wxID_RADIO_BOX, "",
    ...
    // Preview
    wxStaticBoxSizer* m_staticbox = new wxStaticBoxSizer(new wxStaticBox(this, wxID_ANY, "&Preview"), wxVERTICAL);
    ...
    wxButton* itemCancelButton = new wxButton(itemPanel5, wxID_CANCEL);
...
void mmUnivCSVDialog::OnExport(wxCommandEvent& /*event*/)
{
    ...
    *log_field_ << date_format << ("\n");
    ...
    if (fromAccountID > 0)
    {
                ...
                for (std::vector<int>::const_iterator sit = csvFieldOrder_.begin(); sit != csvFieldOrder_.end(); ++ sit)
                {
                    switch (*sit)
                    {
                        ...
                        case UNIV_CSV_AMOUNT:
                            buffer << inQuotes(wxString::Format(("%.2f"), amount), delimit);
                            break;
                        ...
                        case UNIV_CSV_WITHDRAWAL:
                            buffer << inQuotes(wxString::Format(("%.2f"), (amount > 0 ? 0.0 : amount)), delimit);
                            break;
                        case UNIV_CSV_DEPOSIT:
                            buffer << inQuotes(wxString::Format(("%.2f"), (amount > 0 ? amount : 0.0)), delimit);
                            break;
        ...
        wxString msg = wxString::Format(("Transactions exported: %ld"), numRecords);
...
void mmUnivCSVDialog::update_preview()
{
    ...
    {
        wxString date_format = mmDBWrapper::getInfoSettingValue(db_, ("DATEFORMAT"), mmex::DEFDATEFORMAT);
        wxString acctName = m_choice_account_->GetStringSelection();
        int fromAccountID = core_->getAccountID(acctName);
                    ...
                    for (std::vector<int>::const_iterator sit = csvFieldOrder_.begin(); sit != csvFieldOrder_.end(); ++ sit)
                    {
                        ...
                        switch (*sit)
                        {
                            ...
                            case UNIV_CSV_PAYEE:
                                ...
                                break;
                            case UNIV_CSV_AMOUNT:
                                text << inQuotes(wxString::Format(("%.2f"), amount), delimit);
                                break;
                            case UNIV_CSV_CATEGORY:
                            ...
                            case UNIV_CSV_NOTES:
                                ...
                                break;
                            case UNIV_CSV_WITHDRAWAL:
                                text << inQuotes(wxString::Format(("%.2f"), (amount > 0 ? 0.0 : amount)), delimit);
                                break;
                            case UNIV_CSV_DEPOSIT:
                                text << inQuotes(wxString::Format(("%.2f"), (amount > 0 ? amount : 0.0)), delimit);
                                break;
                            case UNIV_CSV_DONTCARE:
                            default:
                                break;
